...
(defun DFA_VAL_ID(token)
	...
	(if (= value 1)(progn
		(if (eq value_list nil) 
			(push (parse-integer token) value_list)
			(push (parse-integer token) (cdr (last value_list)))
			)
	 	(return-from DFA_VAL_ID "VALUE")
	 )
	)
	(if (= indentifier 1)(progn
		 ( if (eq identifier_list nil) 
			(push token identifier_list)
			(push token (cdr (last identifier_list)))
			)
		( return-from DFA_VAL_ID "IDENTIFIER")
		)
	)
...
(defun printRetToken(token)
	...
	(if (eq token_list nil )
		(push (read-from-string token) token_list)
		(push (read-from-string token) ( cdr (last token_list)))
		)
...
(defun IS_KW(token)
  (cond
    ...
    ( (string= token "true") (progn
						    	(if (eq value_list nil) 
									(push 'true value_list)
									(push 'true (cdr (last value_list)))
    								)
						    	(return-from IS_KW "KW_TRUE")
    						)
    )
    ( (string= token "false") 
    						(progn
						    	(if (eq value_list nil) 
									(push 'false value_list)
									(push 'false (cdr (last value_list)))
    								)

						    	(return-from IS_KW "KW_FALSE")
    						)
     )
    (t (return-from IS_KW ""))
  )
